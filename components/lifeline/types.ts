...
/**
 * What the legend can switch. The three event tracks, plus the people
 * row — the faces are their own thread rather than a subdivision of the
 * work one, since an influence is rarely confined to a job.
 */
export type LifelineFilterKey = LifelineTrack | "influence"

export interface LifelineCompany {
  id: CompanyIconId
  name: string
  /**
   * Which track this badge belongs to. When set, the track filter hides
   * the badge with its track. Without it the badge falls back to
   * following whether the year has any visible event left at all.
   */
  track?: LifelineTrack
}
...
export interface LifelinePhoto extends LifelineEventImage {
  ...
  width?: number
  /** Which track this photograph belongs to; see `LifelineCompany.track`. */
  track?: LifelineTrack
}
...
/**
 * Which thread of the story an event belongs to. Rendered as a small
 * coloured dot beside the text and keyed by the footer legend, so the
 * strands stay legible as they braid through the same years.
 *
 * `tech` covers consoles, computers, phones, connections, and operating
 * systems — one thread, not two. Splitting games from hardware implied a
 * distinction that isn't really there.
 */
export type LifelineTrack = "tech" | "work" | "life"
...
export interface LifelineLegendItem {
  /** An event track, or "influence" for the people row. */
  type: LifelineTrack | "influence"
  label: string
}
